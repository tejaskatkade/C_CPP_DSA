...
void main() { 
       ...
       void fifo(char string[],int frameSize,int count) { 
              ...
              for(cnt=0;cnt<count;cnt+=2)	{ 
                     ...
                     if(flag==0){ 
                            faults++; 
                            if(q.end<frameSize) {	//Frame has empty slots 
...
void optimal(char string[],int frameSize,int count){ 
       ...
       for(cnt=0;cnt<count;cnt+=2){	//String[] includes spaces 
              printf("\n\n\t%c",string[cnt]); 
              flag=0; 
              for(cnt2=0;cnt2<q.end;cnt2++) {	//check for existing data in pages 
              ...
              if(flag==0) { 
                     ...
                     for(cnt2=0;cnt2<q.end;cnt2++) {	//Reset reference distances 
                            distance[cnt2]=0; 
                     }       
                     for(selector=0;selector<q.end;selector++) {	//Calculate distance of next reference from current position 
                            for(cnt2=cnt;cnt2<count;cnt2+=2){         //String[] includes spaces   
                                   if(string[cnt2]==q.data[selector][0]) { 
                                          distance[selector]=cnt2/2; 
                                          break; 
                                   } 
                                   if(distance[selector]==0) {   //No further reference 
              ...
              else {	//Data exists in page frame 
                     printf("\t  "); 
                     for(cnt2=0;cnt2<q.end;cnt2++) { 
                            printf("%c   ",q.data[cnt2][0]); 
                     }       
